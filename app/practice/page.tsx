...
type AppSpeechRecognition = {
  ...
  abort: () => void;
  onresult:
    | ((event: AppSpeechResultEvent) => void)
    | null;
  onend: (() => void) | null;
  onerror:
    | ((event: AppSpeechErrorEvent) => void)
    | null;
};

type AppSpeechRecognitionConstructor =
...
We intentionally grade ONLY finalized
speech. Interim speech is not stored.

When mobile Chrome restarts recognition
after a pause, it can repeat several words
from the previous session. This function
merges the new finalized chunk into the
existing transcript.

It accepts:
- exact overlap of any length
- fuzzy overlap of 4+ words when all but
  at most one word match

That handles restart boundaries such as:
...
The repeated boundary is removed instead
of being counted as extra speech.
-------------------------------- */

function wordsAreClose(
  first: string,
  second: string
): boolean {
  if (
  ...
  return getSimilarity(first, second) >= 0.8;
}

function mergeFinalSpeech(
  existing: string,
  incoming: string
): string {
  const existingWords =
  ...
  for (
    ...
    const existingSuffix =
      existingWords.slice(-size);

    const incomingPrefix =
      incomingWords.slice(0, size);
...
/* --------------------------------
Sequence-Aware Verse Comparison

The old one-word lookahead could get
"off track" after one duplicated phrase.
This dynamic-programming alignment can
resynchronize after missing/extra words.
...
function PracticeContent() {
  ...
  /*
   * ONLY finalized speech is kept here.
   * There is intentionally no permanent
   * interim transcript.
   */
  ...
  function startRecognitionInstance(
    ...
    /*
     * We no longer need live interim speech.
     * This reduces the amount of unstable
     * recognition data the app has to handle.
     */
    ...
    recognition.onend = () => {
      ...
      /*
       * Mobile browsers may end one speech
       * recognition session after a pause.
       *
       * While the child has not clicked
       * Done Speaking, quietly start another
       * session and continue collecting finals.
       */
      if (
        shouldKeepListeningRef.current
      ) {
    ...
    recognition.onerror = (
      ...
      /*
       * no-speech commonly accompanies the
       * browser ending a paused session.
  ...
  function stopListening() {
    ...
    /*
     * Keep this session ID alive briefly so
     * a final result delivered by stop()
    ...
    /*
     * Give the browser time to deliver the
     * final result triggered by stop().
     */
    setTimeout(() => {
      recognitionRef.current = null;
  ...
  const listeningPanel = (
    <div className="mt-2">
      ...
      {!score && (
        <div className="mt-2 rounded-xl border border-gray-300 bg-gray-50 px-3 py-3 text-center">
          <p className="text-xs font-bold text-gray-700">
            {isListening
              ? "🎤 Listening… Take your time."
              : "Your answer will be checked after you press Done Speaking."}
          </p>

          <p className="mt-1 text-[10px] text-gray-600">
            The live transcript is hidden so temporary speech-recognition guesses do not distract from practice.
          </p>
        </div>
      )}
    </div>
  ...
  const transcriptResult =
    score && spokenText ? (
      <div className="mt-2 rounded-xl border border-gray-300 bg-gray-50 px-3 py-2">
        ...
        <p className="mt-0.5 text-xs leading-5 text-gray-900">
          {spokenText}
        </p>
      </div>
    ) : null;
